...
<?php while ( have_posts() ) : the_post(); ?>
		...
		<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
			<h2 class="entry-title"><a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'Permalink to %s', 'twentyten' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"><?php the_title(); ?><?php if(get_post_meta($post->ID, "Date", $single = true) != "") {
	...
	<?php if ( is_archive() || is_search() ) : // Only display excerpts for archives and search. ?>
			<?php if (is_category( array ('core_faculty','adjunct_faculty') ) ): ?>
				<?php get_template_part( 'loop', 'core_adjunct' ); ?>
			<?php else: ?>
